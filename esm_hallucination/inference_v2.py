...
def get_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="scaffold length, eg.25,25",
    )
    parser.add_argument(
        "--motif_id",
        type=str,
        default=None,
        help="motif id in reference, eg.P254-277",
    )
    parser.add_argument(
        "--atom",
        type=str,
        default=None,
        help="atom considered when calculating RMSD",
    )
    parser.add_argument(
        "--num-recycles",
        type=int,
...
def main(model,output_fasta, num, motif):
    ...
    for headers, sequences in batched_sequences:
        ...
        all_coord = np.empty(shape=(0,3), dtype = float)
        for header, seq, pdb_string, mean_plddt, ptm in zip(
            ...
            for line in pdb_lines:
                ...
                all_coord = np.concatenate((all_coord,pos),0)

            for res in plddt.keys():
                plddts.append(np.mean(plddt[res]))                          
    # save_path = output_file
    return output_file,pdb_string, coord, all_coord, mean_plddt, plddts
