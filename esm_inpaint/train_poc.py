...
def load_model_optimizer(args,path="/root/.cache/torch/hub/checkpoints/esmfold_3B_v1.pt"):
    ...
    model = modules.esm_inpaint(cfg,chunk_size=args.chunk_size,augment_eps=args.noise,patern=args.parameter_pattern) # make an instance
...
def ddp_main(args):
    ...
    loader_train = customize_data.StructureDataloader(train_set,batch_size=args.batch_size,shuffle=False,sampler=train_sampler)
    ...
    model_data = torch.load(str(model_path), map_location=device) #读取一个pickle文件为一个dict
    cfg = model_data["cfg"]["model"]
    model = modules.esm_inpaint(cfg,chunk_size=args.chunk_size,augment_eps=args.noise,patern=args.parameter_pattern) # make an instance
    model_state = model_data["model"]
    model.esmfold.load_state_dict(model_state, strict=False)
    ...
    model = DDP(model,device_ids=[local_rank], output_device=local_rank) #DDP方式初始化模型,这种方式会在模型的key上带上"module"
    ...
    for epoch in range(args.epochs):
        ...
        for iteration, batch in enumerate(loader_train):
            batch = utils.move_batch(batch,device)
            optimizer.zero_grad()
            output = ddp_model(coord=batch['mask_coord'],mask=(batch['padding_mask']).to(torch.float32),S=batch['mask_seq'])
            # seq loss (nll)
            log_pred = output['log_softmax_aa']
            padding_mask = batch['padding_mask']
            loss_seq_token, loss_seq = utils.loss_nll(batch['seq'], log_pred, padding_mask)
            ...
            total_loss_seq += loss_seq.item()
            total_loss_fape += fape_loss.item()
            if iteration % log_interval == 0 and iteration > 0 and local_rank == 0:
                ms_per_batch = (time.time() - start_time) * 1000 / log_interval
                cur_loss = total_loss / log_interval
                cur_loss_seq = total_loss_seq / log_interval
                cur_loss_fape = total_loss_fape / log_interval
                cur_loss_ppl = np.exp(cur_loss_seq)
                print(f'| epoch {epoch:3d} | {iteration:5d}/{len(loader_train):5d} batches | Rank {local_rank}|'
                    f'loss {cur_loss:5.2f} | ppl {cur_loss_ppl:5.2f} | {cur_loss_fape:5.2f}')
                total_loss = 0
                total_loss_seq = 0
                total_loss_fape = 0
