...
class StructureDataset(Dataset):
    def __init__(self,jsonl_file,truncate=None, max_length=500,low_fraction=0.4,high_fraction=0.9):
        ...
        for entry in dataset:
            ...
            bert_mask_fraction = torch.tensor([np.random.uniform(low=low_fraction, high=high_fraction),])
            bert_mask = []
            ...
            bert_mask = torch.tensor(bert_mask) # 0.0, mask; 1 unmask
            self.data.append({
                "coords":coord,
                "seq":seq,
...
def batch_collate_function(batch):
    ...
    Shape:
        Output:
            ...
            padding_mask_batch       [B, L]  dtype=torch.bool ; 0 represents mask, 1 no mask
    """
    ...
    bert_mask_batch = utils.CoordBatchConverter.collate_dense_tensors([i['bert_mask'] for i in batch],False)
