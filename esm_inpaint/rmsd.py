...
def motif_extract(position, array, atom_part="all"):
    ...
    for i in position:
        ...
        if atom_part == "all":
            motif_array.append(array[(array.chain_id == chain_id) & (
                array.res_id <= end) & (array.res_id >= start) & (array.hetero == False)])
        elif atom_part == "ca":
            ...
            motif_array.append(array[(array.chain_id == chain_id) & (array.res_id <= end) & (array.res_id >= start) & (array.hetero == False) & (
                (array.atom_name == "N") | (array.atom_name == "CA") | (array.atom_name == "C") | (array.atom_name == "O"))])
...
def compute_rmsd(name, motif_native_pos, option=None, num_des=1000, atom_part="ca"):
    ...
    motif_AtomArray = motif_extract(
        motif_native_pos, array, atom_part=atom_part)
    ...
    for stage in ["T1"]:
        ...
        for pdb in range(0, num_des):
            motif_region_des = convert_motif(json.loads(
                motif_region[stage][pdb].replace("'", "\"")))
            if option is None:
            ...
            des_AtomArray = motif_extract(
                motif_region_des, des, atom_part=atom_part)
    ...
    # for i in range(len(rmsd['T1'])):
    #     if rmsd['T1'][i] < 1.0:
    #         print(f"{i},{rmsd['T1'][i]}")
    print(f"{name},{np.min(rmsd['T1'])}/{idx}/{len(rmsd['T1'])}")
    # print(f"{name},{np.min(rmsd['T1'])}/{len(rmsd['T1'])},{idx}")
...
if __name__ == "__main__":
    ...
    # compute_rmsd("5trv","A45-65","5trv_long",num_des=1000,atom_part="backbone")
    ...
    compute_rmsd("1BCF", "A92-99,A123-130,A47-54,A18-25",
                 num_des=20000, atom_part="backbone")

    compute_rmsd("1QJG", "A38,A14,A99", num_des=35000, atom_part="backbone")
    ...
    # compute_rmsd("4JHW","F196-212,F63-69")

    compute_rmsd("4ZYP", "A422-436", num_des=1000, atom_part="backbone")

    compute_rmsd("5WN9", "A170-189", num_des=1000, atom_part="backbone")
